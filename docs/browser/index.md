...
![image](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/786f4691c01d425f9066b831b478f5c2~tplv-k3u1fbpfcp-zoom-1.image)
...
![image](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/a1b63f10a8514c6f8e0ef65cfad3fafe~tplv-k3u1fbpfcp-zoom-1.image)
...
`注意`：GUI渲染线程和JS引擎线程是互斥的，当JS引擎线程执行时，GUI渲染线程会被挂起。这是因为JavaScript是可操纵DOM的，如果在修改这些元素属性同时渲染界面（即JS线程和UI线程同时运行），那么渲染线程前后获得的元素数据就可能不一致了。所以如果JS执行的时间过长，会造成页面的渲染不连贯，导致页面渲染加载阻塞。
...
解析HTML标记并构建DOM树。DOM树的构建是逐行进行的，先构建document节点，然后逐级往下构建。在HTML解析过程中如果遇到script标签，就会暂停HTML标签的解析，去解析和执行JS。
...
通过遍历DOM树和CSSOM规则树，构造渲染树。设置了`display： none` 的元素不会出现在渲染树上，伪元素会出现在渲染树上。
...
![image](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/3be01135ad6e43e4bd45c83d3e87b326~tplv-k3u1fbpfcp-zoom-1.image)
...
![业务流程图1.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/6e11d7a53a644162a329cb5664f78592~tplv-k3u1fbpfcp-zoom-1.image)
