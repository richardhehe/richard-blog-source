...
如上图所示 pnpm 安装了 `bar@1.0.0`这个依赖，它软链接方式指向了 `.pnpm` 中的 `bar@1.0.0`，这个又硬链接指向了 `.pnpm store` 这个仓库，可以通过 `pnpm store path` 这个命令查看电脑中这个仓库的位置，这个仓库存储了所有 `pnpm` 安装的依赖
